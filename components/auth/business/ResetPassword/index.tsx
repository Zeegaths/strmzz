...
/**
 * Renders a form for resetting the business account password.
 */
const BusinessResetPasswordForm = () => {
  ...
  const onSubmit = async (data: ResetPasswordInput) => {
    try {
      console.log('Business reset password email:', data.email);

      // TODO: Replace with actual API call
      // await sendBusinessPasswordResetEmail(data.email);

      await new Promise((resolve) => setTimeout(resolve, 2000)); // Simulate delay

      reset();
      router.push('/auth/business/confirm-password-reset');
    } catch (error) {
    ...
    <div className="shadow-authCardShadow md:w-[380px] w-full rounded-[16px] bg-white border border-[#E5E7EB] flex flex-col gap-4 items-center py-8 px-6 relative">
      ...
      <div className="w-full flex flex-col gap-3">
        ...
        <p className="font-poppins font-[400] text-[14px] leading-[24px] text-[#58556A] text-center px-3">
          No worries! Enter your business email to reset your password and regain access.
        </p>
      </div>
...
export default BusinessResetPasswordForm;
