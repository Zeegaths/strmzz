...
interface ElectricityFormProps { onSuccess: () => void }

const ElectricityForm = ({ onSuccess }: ElectricityFormProps) => {
    const { address } = useAccount()
    const [payStep, setPayStep] = useState<PayStep>('form')
    const [formData, setFormData] = useState<ElectricityInput | null>(null)

    const { register, handleSubmit, control, watch, formState: { errors, isSubmitting } } = useForm<ElectricityInput>({
        resolver: zodResolver(electricitySchema),
        mode: 'onChange',
        defaultValues: { electricityProvider: '', meterType: '', meterNumber: '', amount: '', token: 'USDC', phoneNumber: '', email: '' },
    })
    ...
    useEffect(() => { if (payConfirmed) { setPayStep('done'); toast.success('Electricity purchase successful!'); setTimeout(() => onSuccess(), 1500) } }, [payConfirmed, onSuccess])
    ...
    const handlePay = () => {
        ...
        const preference = JSON.stringify({ type: 'electricity', provider: formData.electricityProvider, meterType: formData.meterType, meter: formData.meterNumber, phone: formData.phoneNumber, email: formData.email, amount: formData.amount })
    ...
    if (payStep === 'pay') return (
        <div className="w-full flex flex-col gap-4">
            <div className="flex flex-col gap-1"><h4 className="font-sora font-[600] text-base text-primary">Confirm Payment</h4><p className="font-poppins text-sm text-[#58556A]">Review your electricity details before paying.</p></div>
            <div className="flex flex-col gap-3 px-4 py-4 bg-[#F9FAFB] rounded-[8px] border border-[#E5E7EB]">
                <div className="flex justify-between"><span className="font-poppins text-xs text-[#9CA3AF] uppercase">Provider</span><span className="font-poppins text-sm font-[500] text-primary">{formData?.electricityProvider}</span></div>
                <div className="flex justify-between"><span className="font-poppins text-xs text-[#9CA3AF] uppercase">Meter Type</span><span className="font-poppins text-sm font-[500] text-primary">{formData?.meterType}</span></div>
                <div className="flex justify-between"><span className="font-poppins text-xs text-[#9CA3AF] uppercase">Meter Number</span><span className="font-poppins text-sm font-[500] text-primary">{formData?.meterNumber}</span></div>
    ...
    if (payStep === 'done') return (
        <div className="w-full flex flex-col items-center gap-4 py-6">
            ...
            <h4 className="font-sora font-[600] text-lg text-primary">Payment Successful!</h4>
            <p className="font-poppins text-sm text-[#58556A] text-center">Electricity units sent to meter {formData?.meterNumber}</p>
        </div>
    )
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="w-full flex flex-col gap-4">
            <div className="w-full flex flex-col gap-2">
                <label className="font-poppins text-[14px] text-[#58556A] leading-[24px]">Electricity provider</label>
                <Controller name="electricityProvider" control={control} render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange}>
                        <SelectTrigger className="w-full h-[44px] rounded-[8px] border bg-[#F9FAFB] shadow-navbarShadow font-poppins text-[14px] text-[#8E8C9C] px-4 focus:border-accent outline-none"><SelectValue placeholder="Select electricity provider" /></SelectTrigger>
                        <SelectContent>{electricityProviders.map((p) => <SelectItem key={p.value} value={p.value}>{p.label}</SelectItem>)}</SelectContent>
                    </Select>
                )} />
                {errors.electricityProvider && <p className="text-red-500 text-[12px] font-poppins">{errors.electricityProvider.message}</p>}
            </div>
            <div className="w-full flex flex-col gap-2">
                <label className="font-poppins text-[14px] text-[#58556A] leading-[24px]">Account/meter type</label>
                <Controller name="meterType" control={control} render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange}>
                        <SelectTrigger className="w-full h-[44px] rounded-[8px] border bg-[#F9FAFB] shadow-navbarShadow font-poppins text-[14px] text-[#8E8C9C] px-4 focus:border-accent outline-none"><SelectValue placeholder="Select meter type" /></SelectTrigger>
                        <SelectContent>{meterTypes.map((t) => <SelectItem key={t.value} value={t.value}>{t.label}</SelectItem>)}</SelectContent>
                    </Select>
                )} />
                {errors.meterType && <p className="text-red-500 text-[12px] font-poppins">{errors.meterType.message}</p>}
            </div>
            <FormInput label="Meter number" id="meterNumber" type="text" placeholder="Meter number" register={register('meterNumber')} error={errors.meterNumber?.message} />
            <div className="grid grid-cols-2 gap-4">
                ...
                <FormInput label="Amount" id="amount" type="text" placeholder="Enter amount" register={register('amount')} error={errors.amount?.message} />
            </div>
